...
/*   Updated: 2021/02/25 12:57:32 by rarias-p         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		texture_err2(t_data *data, int fd)
{
	...
	!(ft_strnstr(data->text_sp, ".xpm", ft_strlen(data->text_sp))))
		...
		close(fd);
	if (0 > (fd = open(data->text_sp2, O_RDONLY)) ||
	!(ft_strnstr(data->text_sp2, ".xpm", ft_strlen(data->text_sp2))))
		return (1);
	else
		close(fd);
...
int		texture_err(t_data *data)
{
	...
	if (!(data->text_ea) || !(data->text_no) || !(data->text_so) ||
		!(data->text_sp) || !(data->text_we) || !(data->text_sp2))
		return (1);
